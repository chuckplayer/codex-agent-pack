...
copy_dir_children "$pack_root/.agents/skills" "$skills_home"
copy_dir_children "$pack_root/.codex/agents" "$codex_home/agents"

mkdir -p "$agent_pack_home/hooks"
copy_dir_children "$pack_root/.codex/hooks" "$agent_pack_home/hooks"
...
if [[ "$install_hooks" -eq 1 ]]; then
  ...
  copy_path "$pack_root/.codex/global-hooks.json" "$hooks_destination"
fi
