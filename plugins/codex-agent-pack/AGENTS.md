...
- `scripts/*.sh` are the supported entrypoints for macOS, Linux, and Windows
  with Git Bash.
- `plugins/codex-agent-pack/` is the generated Codex plugin package built from
  the authoring layout.
- `.agents/plugins/marketplace.json` is the repo-local marketplace entry for
  installing the generated plugin package.
- `README.md` is the user-facing installation and usage guide.
- `docs/INSTALL.md` contains the plugin and script-only install flows.
...
- Treat `.agents/skills`, `.codex/agents`, `.codex/hooks`, and `scripts` as the
  source of truth. Rebuild `plugins/codex-agent-pack` after changing them.
...
The validator checks skills, custom agents, hooks, shell support scripts, the
generated plugin package, and the repo-local marketplace file.

## Plugin Package

Build the repo-local plugin package with:

```bash
bash scripts/build-plugin.sh
```

Install or update the local plugin package with:

```bash
bash scripts/install-codex-plugin.sh --hooks
```

For local update/reinstall testing, add a cachebuster:
